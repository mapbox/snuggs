...
def test_arr_shortname():
    with parsnip.ctx(foo=numpy.ones((2, 2))):
...
def test_arr_shortname_slice():
    with parsnip.ctx(foo=numpy.ones((2, 2))):
        r = parsnip.handleLine('(ra foo 1)')
        assert list(r.flatten()) == [1, 1]
        r = parsnip.handleLine('(ra foo 2)')
        assert list(r.flatten()) == [1, 1]
...
def test_list():
    with parsnip.ctx(foo=numpy.ones((2, 2)), bar=numpy.ones((2, 2))):
        result = parsnip.handleLine('(list (ra foo 1) (ra foo 1) (ra bar 1) (ra bar 1))')
